SHELLData2020.2025 <- read.csv("../data/SHELLData2020-2025.csv",sep=";", dec=".")
NYSEnergy2020.2025 <- read.csv("../data/NYSEnergy2020-2025.csv",sep=";", dec=".")

SHELL.P <- SHELLData2020.2025$X
NYSE.P <- NYSEnergy2020.2025$Ultimo
n <- length(SHELL.P)

SHELL.R <- SHELL.P[-1]/SHELL.P[-n]-1
NYSE.R <- NYSE.P[-1]/NYSE.P[-n]-1


plot(NYSE.R, SHELL.R, pch=16)
abline(h=0,v=0)

# OLS
ols <- lm(SHELL.R ~ NYSE.R)
...
ks <- ksmooth(NYSE.R, SHELL.R, bandwidth = 0.28)  
...
plot(density(residuals,n=60), main="Density of residuals",col="red", lwd=2)
...
# Plot of Neyman Test
x_vals <- seq(-9 * SE, 14 * SE, length.out = 500)
t_vals <- (x_vals - 0) / SE
...
x_vals <- seq(B0 - 6 * SE, B0 + 6 * SE, length.out = 500)

# Funzione di densità t centrata in B0
...
# Line of critical region
abline(v = K.alpha3, col = "red", lty = 2, lwd = 2)

# Line of estimate of beta
